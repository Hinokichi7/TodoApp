...
async orderByDatatByQuery() {
    const currentPage = await firebase.firestore().collection('users').orderBy('updateAt').limit(3).get();
    const nextPage = await firebase.firestore().collection('users').orderBy('updateAt').startAfter(currentPage.docs[currentPage.size - 1]).limit(3).get();
}
