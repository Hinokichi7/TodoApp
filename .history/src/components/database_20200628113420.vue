<template>
    <v-container>
        <v-row>
            ...
            <v-col cols="4">
                <v-btn color="primary" @click="createDocumentById">作成</v-btn>
...
todo: string[] = ['todo1', 'todo2']
  ...
  const responce = await firebase.firestore().collection('users').add({ name: 'title', value: 3 });
  console.log(responce);
}

  async createDocumentByRandomId_2() {// eslint-disable-line
  await firebase.firestore().collection('users').doc().set({ name: 'priority', value: 4 });
}

  async createDocumentById() {// eslint-disable-line
  await firebase.firestore().collection('users').doc(this.mail).set({ name: 'user', value: this.todo });
}
