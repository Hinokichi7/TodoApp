      ...
      <v-row align="center" justify="center">
        <v-col cols="12" sm="8" md="4">
          <v-card class="elevation-12">
            ...
            <v-card-text>
              <v-form>
                <v-text-field label="e-mail" prepend-icon="mdi-mail" v-model="emailLink"></v-text-field>
              </v-form>
            </v-card-text>
            <v-card-actions>
              <v-spacer></v-spacer>
...
export default class TrySignin extends Vue {
    ...
    sendMail() {
      firebase.auth().sendSignInLinkToEmail(this.emailLink, this.actionCodeSettings)
        .then((responce) => {
        // The link was successfully sent. Inform the user.
        // Save the email locally so you don't need to ask the user for it again
        // if they open the link on the same device.
          window.localStorage.setItem('emailForSignIn', this.emailLink);
        })
        .catch((error) => {
