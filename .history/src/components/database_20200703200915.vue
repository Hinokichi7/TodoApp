...
// }
async getDataNextPage() {// eslint-disable-line
  let currentPage = await firebase.firestore()// eslint-disable-line
    .collection('users')
    .orderBy('name')
    .limit(3)
    .get();
  let lastVisible = currentPage.docs[currentPage.size - 1];// eslint-disable-line
  let nextPage = await firebase.firestore()// eslint-disable-line
    .collection('users')
    .orderBy('name')
