    ...
    <v-col cols="12" sm="8" offset-sm="2">
        ...
        <v-app-bar-nav-icon></v-app-bar-nav-icon>
            <v-toolbar-title>Todo List</v-toolbar-title>
              <v-spacer/>
              <v-btn class="mx-2" fab dark color="#0288D1" small @click="showForm(true, todo)">
...
async selectDocument() {
  const qSnapshot = await this.db
    ...
    .where('selected', '==', true)
    .get();
  qSnapshot.forEach((dSnapshot) => console.log(dSnapshot.data()));
}
...
selected(todo: ToDo) {
  ...
  this.showForm(false, todo);
  this.$store.commit('todos/selected', this.selectedTodo);
}

showForm(reset: boolean, todo: ToDo) {
  this.formCount += 1;
  if (reset) {
    // this.$store.dispatch('todos/resetSelected');
    todo.selected = false;// eslint-disable-line
  }
  this.dialog = true;
}
