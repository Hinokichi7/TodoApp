  ...
  <v-row>
    ...
    <v-col cols="12" sm="8" offset-sm="2">
        <v-container class="lighten-2">
          ...
          <v-col cols="3">
            <v-select
              :items="priorityItems" label="Pick Priority" multiple v-model="targetPriority" @change="filterPriority"
            ></v-select>
          </v-col>
          <v-col cols="4">
            <v-select
              :items="progressItems" label="Pick Progress" multiple v-model="targetProgress" @change="filterProgress"
            ></v-select>
          </v-col>
          <v-col cols="4">
            <v-select
              :items="sortItems" label="Choose Sortitem" v-model="sortOption" @change="sortTodo"
            ></v-select>
          </v-col>
...
export default class TodoList extends Vue {
  ...
  sortTodo() {
    ...
    this.todos.sort((a: any, b: any) => b.sortOp - a.sortOp);
  }
  // async queryPriority() {
  //   const qSnapshot = await this.db
